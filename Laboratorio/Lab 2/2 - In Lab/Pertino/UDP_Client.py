...
try:
    # Dobbiamo indicare come parametro la dimensione del buffer (in byte).
    serverReply, serverAddress = clientSocket.recvfrom(2048)

    # Decodifico il messaggio e lo stampo a schermo.
    serverReply = serverReply.decode('utf-8')
    print(serverReply)
except:
    print("Errore. Timeout scaduto. Prova tra 5 minuti.")
finally:
    # Chiudo la socket precedentemente aperta, concludendo l'applicazione.
    clientSocket.close()
